...
class MyCanvas(Canvas):
    ...
    def create_line_h(self, x, y, l):
        self.create_line(x, y, x + 2*l, y, width=self.hLineWidth)
    ...
    def draw(self, cvs, x, y, hScale, vScale):
        if self.size > 1:
            ...
            for i in range(self.size // 4):
                cvs.create_segment_v(x + (i + 0.5) * hScale, y + (2*i)* vScale, (self.size // 2) * vScale)
    ...
    def draw(self, cvs, x, y, hScale, vScale):
        if self.size > 1:
            ...
            for i in range(self.size-1):
                cvs.create_segment_v(x , y + (2*i +1 )* vScale,  vScale)
...
class BitonicSorter:
    ...
    def draw(self, cvs, x, y, hScale, vScale):
        if self.size > 1:
            ...
            self.subSorter.draw(cvs, x + self.halfCleaner.hNum() * hScale//2, y,
                                hScale, vScale)
            self.subSorter.draw(cvs, x + self.halfCleaner.hNum() * hScale//2, y + (self.size // 2) * vScale,
                                hScale, vScale)
...
class Merger:
    ...
    def draw(self, cvs, x, y, hScale, vScale):
        if self.size > 1:
            ...
            for i in range(self.size // 2):
                if self.size==2:
                    cvs.create_segment_v(x + (i + 1.5) * hScale, y + i * vScale,  vScale)
                    break

                if i % 2 == 0:
                    cvs.create_segment_v(x + (i + 1.5) * hScale, y + i * vScale, (self.size - 2 - 2 * i) * vScale)  
                else:
                    cvs.create_segment_v(x + (i + 1.5) * hScale, y + i * vScale, (self.size  - 2 * i) * vScale)

            self.subSorter.draw(cvs, x + (self.size // 2+1) * hScale, y, hScale, vScale)
            self.subSorter.draw(cvs, x + (self.size // 2+1) * hScale, y + (self.size // 2) * vScale, hScale, vScale)

            self.subSorter.draw(cvs, x + (3*self.size//4) * hScale, y + vScale, hScale, vScale)
            self.subSorter.draw(cvs, x + (3*self.size//4) * hScale, y + (self.size // 2+1) * vScale, hScale, vScale)

            self.cl.draw(cvs, x + (self.size-0.5) * hScale, y, hScale, vScale)
...
class Sorter:
    ...
    def draw(self, cvs, x, y, hScale, vScale):
        if self.size > 1:
            ...
            self.merger.draw(cvs, x + (self.subSorter.hNum()-1)* hScale, y, hScale, vScale)
